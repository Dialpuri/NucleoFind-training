...
for path in os.scandir("/vault/NucleoFind-training-pr/unrefined_reserved_data"):
    pdb = path.name
    unrefined_base_path = "/vault/NucleoFind-training-pr/unrefined_reserved_data"
    data_dir = os.path.join(path, "0", f"{pdb}_0.mtz")
    if os.path.exists(data_dir):
        unref_res_data.append((data_dir, f"/vault/NucleoFind-training-pr/data/protein_na_reserved/pdb/{pdb}.pdb", pdb,  f"/vault/NucleoFind-training-pr/unrefined_reserved_maps/{pdb}", radius))

with multiprocessing.Pool(4) as p:
    x = list(tqdm(
        p.imap_unordered(captured_worker, unref_res_data), total=len(unref_res_data)
    ))
